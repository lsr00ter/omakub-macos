...
if [ -f "$THEME_DIR/background.jpg" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            echo "Desktop wallpaper set to Rose Pine background (via database method)"
        else
            echo "Warning: Could not set Rose Pine wallpaper. You can set it manually in System Preferences > Desktop & Screen Saver"
        fi
    fi
else
